class Admin::NewslettersController < Admin::BaseController
	...
	def new
		js 'form'
		@orators = User.joins(:orator).distinct
    @owners = User.joins(:cards).distinct
		@resps = (User.joins(:card_users).where(card_users: {user_validated: true, card_validated: true}) - @owners).distinct
		@users = (User.users - @orators - @owners - @resps).distinct
	end

	def create
		js 'form'
		emails = User.joins(:newsletters).where(email: params[:emails], newsletters: { id: params[:options] }).distinct.pluck(:email)
